...
binary_dist/dist.go: sim/web/dist.go.tmpl
	mkdir -p binary_dist/80
	touch binary_dist/80/embedded
...
binary_dist: $(OUT_DIR)/.dirstamp
	...
	rm binary_dist/80/lib.wasm
	rm -rf binary_dist/80/assets/db_inputs
	rm binary_dist/80/assets/database/db.bin
	rm binary_dist/80/assets/database/leftover_db.bin
...
# go build only considers syso files when invoked without specifying .go files: https://github.com/golang/go/issues/16090
	...
	mv ./sim/web/wowsimwotlk-windows.exe ./release/80/wowsimwotlk-windows.exe
	mv ./cmd/wowsimcli/wowsimcli-windows.exe ./release/80/wowsimcli-windows.exe

release: wowsimwotlk wowsimwotlk-windows.exe
	GOOS=darwin GOARCH=amd64 GOAMD64=v2 go build -o release/80/wowsimwotlk-amd64-darwin -ldflags="-X 'main.Version=$(VERSION)' -s -w" ./sim/web/main.go
	GOOS=darwin GOARCH=arm64 go build -o release/80/wowsimwotlk-arm64-darwin -ldflags="-X 'main.Version=$(VERSION)' -s -w" ./sim/web/main.go
	GOOS=linux GOARCH=amd64 GOAMD64=v2 go build -o release/80/wowsimwotlk-amd64-linux   -ldflags="-X 'main.Version=$(VERSION)' -s -w" ./sim/web/main.go
	GOOS=linux GOARCH=amd64 GOAMD64=v2 go build -o release/80/wowsimcli-amd64-linux --tags=with_db -ldflags="-X 'main.Version=$(VERSION)' -s -w" ./cmd/wowsimcli/cli_main.go
# Now compress into a zip because the files are getting large.
	cd ./release/80/ && zip wowsimwotlk-windows.exe.zip wowsimwotlk-windows.exe && zip wowsimwotlk-amd64-darwin.zip wowsimwotlk-amd64-darwin && zip wowsimwotlk-arm64-darwin.zip wowsimwotlk-arm64-darwin && zip wowsimwotlk-amd64-linux.zip wowsimwotlk-amd64-linux && zip wowsimcli-amd64-linux.zip wowsimcli-amd64-linux && zip wowsimcli-windows.exe.zip wowsimcli-windows.exe
...
ifeq ($(WATCH), 1)
	ulimit -n 10240 && air -tmp_dir "/tmp" -build.include_ext "go,ts,js,html,tsx" -build.bin "npx" -build.args_bin "http-server $(OUT_DIR)/../../.." -build.cmd "make" -build.exclude_dir "dist,node_modules,tools"
else
