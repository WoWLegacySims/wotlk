...
import (
	"time"
"""
    if (cls != "core"): output += '    "github.com/WoWLegacySims/wotlk/sim/core"\n'
    output += """)
"""
    return output
...
def GenSpellInfo(name: str, ids: List[int],package: str):
    output = f"var {''.join(CleanName(name).split())}Infos = []*{package}SpellInfo{{\n"
    level = 0
    for id in ids:
        output += "{"
        dbc = GetDBC(id)
        ...
        output += f"SpellID:{spellInfo.SpellID},MinLevel:{spellInfo.MinLevel},MaxLevel:{spellInfo.MaxLevel},Duration:{spellInfo.Duration},Period:{spellInfo.Period},CastTime: {spellInfo.CastTime} * time.Millisecond,BaseCost: {spellInfo.BaseCost},Effects: [3]{package}SpellEffect{{"
...
def GenSpells(cls: str):
    ...
    for spell in SPELLS[cls]:
        output += GenSpellInfo(spell, SPELLS[cls][spell],package)
