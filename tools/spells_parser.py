...
SPELLS = {
    ...
    "druid": {
        ...
        "Gift of the Wild": [21849,21850,26991,48470],
        ...
        "HurricaneDamage": [42231,42232,42233,42230,48466],
        ...
        "Starfall Aura": [50286,53196,53197,53198],
        "Starfall Direct": [50288,53191,53194,53195],
        "Starfall Splash": [50294,53188,53189,53190],
        "Starfire": [2912,8949,8950,8951,9875,9876,25298,26986,48464,48465],
        "Swipe (Bear)": [779,780,769,9754,9908,26997,48561,48562],
        "Thorns": [467,782,1075,8914,9756,9910,26992,53307],
        "Tiger's Fury": [5217,6793,9845,9846,50212,50213],
        "Typhoon": [50516,53223,53225,53226],
    ...
    "hunter": {
        ...
        "VolleyEffect": [42243,42244,42245,42234,58432,58433],
        #pet
    ...
    "mage": {
        ...
        "Arcane Missiles Damage": [7268,7269,7270,8419,8418,10273,10274,25346,27076,38700,38703,42844,42845],
        ...
        "Replenish Mana": [5405,10052,10057,10058,27103,42987],
    ...
    "rogue": {
        ...
        "Deadly Poison": [2818,2819,11353,11354,25349,26968,27187,57969,57970],
        ...
        "Instant Poison": [8680,8685,8689,11335,11336,11337,26890,57964,57965],
        "Mutilate": [1329,34411,34412,34413,48663,48666],
        "MutilateMH": [5374,34414,34416,34419,48662,48665],
        "MutilateOH": [27576,34415,34417,34418,48661,48664],
        ...
        "Wound Poison": [13218,13222,13223,13224,27189,57974,57975],
	},
    "shaman": {
        ...
        "Flametonge Totem Effect": [52109,52110,52111,52112,52113,58651,58654,58655],
        ...
        "Frost Resistance": [8182,10476,10477,25559,58742,58744],
        "Frost Shock": [8056,8058,10472,10473,25464,49235,49236],
        "Frostbrand Weapon": [8033,8038,10456,16355,16356,25500,58794,58795,58796],
        "Frostbrand Attack": [8034,8037,10458,16352,16353,25501,58797,58798,58799],
        ...
        "Lightning Shield Effect": [26364,26366,26367,26369,26370,26363,26371,26372,49278,49279],
    ...
    "warlock": {
        ...
        "Fel Intelligence": [54424,57564,57565,57566,57567],
        "Blood Pact": [6307,7804,7805,11766,11767,27268,47982]
	},
    "warrior": {
    ...
    "common": {
        "Lifeblood": [55428,55480,55500,55501,55502,55503],
        "Intellect": [8096,8097,8098,12176,33078,43195,48099,48100],
        "Stamina": [8099,8100,8101,12178,33081,43198,48101,48102],
        "Spirit": [8112,8113,8114,12177,33080,43197,48103,48104],
        "Agility": [8115,8116,8117,12174,33077,43194,58450,58451],
        "Strength": [8118,8119,8120,12179,33082,43199,58448,58449],
        "Armor": [8091,8094,8095,12175,33079,43196,58452,58453],
	}
}
...
Off = {
    ...
    "EffectAuraPeriod": 95,
    "EffectMiscValue": 107,
    "EffectMiscValueB": 110,
    "EffectTriggerSpell": 113,
    "EffectPerCombo": 116,
    "Name": 133,
    "Rank": 150,
    "BaseCost": 201,
    "EffectChainAmplitude": 213,
    "Coefficient": 226,
}
class SpellEffect:
    BasePoints: str
...
def GenSpellInfo(name: str, ids: List[int]):
    ...
    for id in ids:
        ...
        spellInfo.Rank = dbc[Off["Rank"]].split(" ").pop()
        spellInfo.ManaCost = dbc[Off["Manacost"]]
        for i in range(0,3):
            effect = SpellEffect()
            ...
            effect.PerCombo = dbc[Off["EffectPerCombo"] + i]
            effect.ChainAmplitude = dbc[Off["EffectChainAmplitude"] + i]
            effect.AuraPeriod = dbc[Off["EffectAuraPeriod"] + i]
            effect.TriggerSpell = dbc[Off["EffectTriggerSpell"] + i]
            effect.MiscValueA = dbc[Off["EffectMiscValue"] + i]
            effect.MiscValueB = dbc[Off["EffectMiscValueB"] + i]
        ...
        for effect in spellInfo.Effects:
            output += f"{{BasePoints: {effect.BasePoints},Die: {effect.Die},LevelScaling: {effect.LevelScaling},Coefficient: {effect.Coefficient},PointsPerCombo: {effect.PerCombo},ChainAmplitude: {effect.ChainAmplitude},AuraPeriod: {effect.AuraPeriod},TriggerSpell: {effect.TriggerSpell},MiscValueA: {effect.MiscValueA}, MiscValueB: {effect.MiscValueB}}},"
